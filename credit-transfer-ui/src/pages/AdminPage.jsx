...
import './FacultyDashboard.css'; // ยืม CSS มาใช้เพื่อให้สวยงามเหมือนกัน

function AdminPage() {
  const { user, logoutUser } = useContext(AuthContext);
  
  // --- State และ Logic ส่วนใหญ่เหมือน DataManagementPage ---
        ...
        <div className="request-card-body">
            <form onSubmit={handleSave}>
                ...
                <div style={{marginTop: '15px'}}>
                    <button type="submit" className="btn btn-primary" style={{backgroundColor: '#dc3545'}}>บันทึก (Admin)</button>
  ...
  return (
    <div className="faculty-dashboard">
      ...
      <main className="dashboard-main">
        ...
        {!isEditing && (
             <button className="btn btn-primary" style={{marginBottom: '20px', backgroundColor: '#dc3545'}} onClick={() => {setFormData({}); setIsEditing(true);}}>
                + เพิ่มข้อมูลใหม่
             </button>
        )}
        ...
        <div className="request-card">
            <table className="course-table">
                ...
                <tbody>
                    {items.length === 0 ? (
                        <tr><td colSpan="3" style={{textAlign:'center', padding:'20px', color:'#999'}}>-- ไม่พบข้อมูล --</td></tr>
                    ) : (
                        items.map(item => (
                            <tr key={item.id}>
                                <td>{item.id}</td>
                                <td>
                                    {item.name && <div style={{fontSize: '1.1rem', fontWeight: 'bold'}}>{item.name}</div>}
                                    
                                    {item.course_code && (
                                        <div>
                                            <span style={{color: '#dc3545', fontWeight: 'bold'}}>{item.course_code}</span> {item.course_name_th} <br/>
                                ...
                                <td>
                                    <button className="btn" style={{marginRight: '5px', background: '#ffc107', color: 'black', padding: '5px 10px', fontSize: '0.8rem'}} onClick={() => {setFormData(item); setIsEditing(true);}}>แก้ไข</button>
                                    <button className="btn btn-reject" style={{padding: '5px 10px', fontSize: '0.8rem'}} onClick={() => handleDelete(item.id)}>ลบ</button>
                                </td>
                            </tr>
                        ))
                    )}
