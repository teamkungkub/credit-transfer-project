...
function StudentDashboard() {
 ...
 const handleSubmit = async (e) => { 
    ...
    // 1. เช็คไฟล์ก่อนเป็นอันดับแรก! (ตามโค้ดเดิม)
    if (!evidenceFile) {
    ...
    const items = Object.keys(selectedCourses).map(id => ({
      ...
      grade: selectedCourses[id].grade || '', // ถ้าไม่กรอกจะเป็นค่าว่าง
    }));
    ...
    // 🔥 [ส่วนที่เพิ่ม] เช็คว่ามีวิชาไหนลืมกรอกเกรดหรือไม่?
    // ถ้า item.grade เป็นค่าว่าง ('') ให้ถือว่ายังไม่กรอก
    ...
    if (incompleteItems.length > 0) {
        ...
        return; // ⛔ สั่งหยุดทันที ไม่ให้ส่งข้อมูล
    }

    // --- ถ้าผ่านด่านข้างบนมาหมดแล้ว ค่อยส่งข้อมูล ---
    try {
      setIsLoading(true);
      ...
      // 🔥 [ส่วนที่แก้ไข] เปลี่ยนจากการ Alert ธรรมดา เป็นการเปลี่ยนหน้าจอ
      // alert('ส่งคำร้องสำเร็จ!'); <-- เอาออก หรือเก็บไว้ก็ได้แต่มันจะขัดกับหน้าจอใหม่
      setIsSuccess(true); // ✅ สั่งให้หน้าจอเปลี่ยนเป็น "กำลังประมวลผล"

      // ส่วนการเคลียร์ค่า (เก็บไว้เหมือนเดิม หรือจะลบออกก็ได้เพราะหน้าจอเปลี่ยนไปแล้ว)
